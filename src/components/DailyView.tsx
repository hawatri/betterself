      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Daily Spent</p>
              <p className={`text-2xl font-bold ${isOverTarget ? 'text-red-600' : 'text-gray-900'}`}>
                {formatCurrency(totalSpent)}
              </p>
              <p className="text-sm text-gray-500">Target: {formatCurrency(dailyTarget)}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-2xl font-bold ${currentDue > 0 ? 'text-orange-600' : 'text-gray-900'}`}>
                {formatCurrency(currentDue)}
              </p>
              {remainingTarget > 0 && (
                <p className="text-sm text-green-600">Can save: {formatCurrency(remainingTarget)}</p>
              )}
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Savings Transferred</p>
              <p className="text-2xl font-bold text-green-600">
                {formatCurrency(dailyData.savingsTransferred || 0)}
              </p>
              {dailyData.borrowed && (
                <p className="text-sm text-blue-600">Borrowed: {formatCurrency(dailyData.borrowed)}</p>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Spending Entries</h3>
            ...
            <div className="space-y-3">
              {dailyData.spending?.map((entry) => (
                ...
                <p className="text-gray-500 text-center py-4">No spending entries yet</p>
              )}
            </div>
          </div>
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {task.createdDate !== formatDate(selectedDate) && (
                      <p className="text-xs text-blue-600">Carried over</p>
                    )}
                  </div>
                  <button
        ...
        <div className="space-y-6">
          ...
          {previousDayData?.notes && (
            <div className="bg-amber-50 rounded-xl shadow-sm p-6 border border-amber-200">
              <div className="flex items-center space-x-2 mb-3">
                ...
                <h3 className="text-lg font-semibold text-amber-800">Yesterday's Notes</h3>
              </div>
              <div className="bg-white p-4 rounded-lg">
                <p className="text-gray-700 whitespace-pre-wrap">{previousDayData.notes}</p>
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            />
            <p className="text-xs text-gray-500 mt-2">Notes are automatically saved</p>
          </div>
        </div>
      </div>
